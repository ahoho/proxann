"""Extracts information from a two topic models provided in a configuration file and saves the output in a JSON file.

It gets the first N topics from the first model (assuming this is a Mallet model, this means these topics are the most representative in the corpues, i.e. larger alpha), and finds the most similar topics in the second model based on the WMD distance between the top words of the topics.
...
from src.jsonfy.topic_json_formatter import TopicJsonFormatter
...
def main():
    ...
    selected_topics = topic_selector.find_closest_by_wmd(all_topic_keys, range(N))
