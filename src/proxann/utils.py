...
def bradley_terry_model(
    ...
    if use_choix:
        ...
        try:
            ...
            params = choix.ilsr_pairwise(n_items, data, alpha=0.001)
...
def extract_info_q1_q3(text, get_label):
    ...
    if get_label:
        label_match = re.findall(
            label_pattern, text, re.DOTALL | re.IGNORECASE)
    ...
    rationale_match = re.findall(
        rationale_pattern, text, re.DOTALL | re.IGNORECASE)
...
def extract_logprobs(pairwise_logprobs, backend, logger):
    ...
    try:
        ...
        log_or_print(f"-- -- Error extracting logprobs: {e}", logger)
        return None
...
def collect_fit_rank_data(
    ...
    for topic_id, group in responses_by_id.items():
        ...
        fit_data = np.array(
            [[doc["fit"] for doc in r["eval_docs"]] for r in group])
        rank_data = 8 - np.array([[doc["rank"] for doc in r["eval_docs"]]
                                 for r in group])  # Reverse rank
...
def compute_correlations_one(
    ...
    for d_us, d_r_llm, d_f_llm in zip(corr_data, rank_llm_data, fit_llm_data):
        ...
        binarized_fit_mv = (np.round(np.mean(fit_data, axis=0))
                            >= fit_threshold).astype(int)
